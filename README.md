...
Displays a single product with name, category, price, and an "Add to Cart" button.
...
Central state manager for all cart interactions. Wraps all API calls and exposes reactive state.

```js
const {
  cart,          // { items, subtotal, discount, total, coupon }
  loading,       // boolean — API request in flight
  cartError,     // string|null — last API error message
  couponStatus,  // { message, type } — 'success' | 'error'
  orderState,    // { orderId, loading, error }
  addItem,       // (product) => void
  updateQty,     // (itemId, quantity) => void
...
The cart is automatically fetched on mount and whenever `sessionId` changes.

---

## Testing

Tests are written with **Vitest** and **React Testing Library**. MSW intercepts all HTTP requests during tests so no real backend is required.
...
| File | What it tests |
|------|---------------|
| `src/components/CartSidebar/CartSidebar.test.jsx` | Rendering, item interactions, coupon feedback, order states |
| `src/components/CheckoutButton/CheckoutButton.test.jsx` | Button states, loading spinner, order confirmation |
| `src/components/CouponInput/CouponInput.test.jsx` | Input behaviour, apply action, success/error feedback |

---

## Mock Server (MSW)

Located in `src/mocks/`. Provides realistic API responses without a backend.

**Shared mock data** (`handlers.js`):

```js
MOCK_SESSION     // 'test-session-123'
MOCK_ITEM        // Single cart item (Wireless Headphones)
MOCK_CART        // Cart with one item, no coupon
MOCK_CART_EMPTY  // Cart with no items
MOCK_CART_COUPON // Cart with SAVE50 coupon applied (₹50 discount)
MOCK_ORDER       // Placed order object with status 'PLACED'
```
...
- `POST /cart/:sessionId/coupon` (handles valid, `INVALID`, and `DUPLICATE` codes)
- `DELETE /cart/:sessionId`
- `POST /orders`

**Test setup** (`tests/setup.js`) starts the MSW server before all tests, resets handlers between tests, and closes the server after all tests complete.

---

## Code Coverage

Coverage is generated using `@vitest/coverage-v8`. Reports are output in `text`, `lcov`, and `html` formats.
...
The HTML report is generated in the `coverage/` directory. Open `coverage/index.html` in a browser to view detailed line-by-line coverage.

---

## Session Management

Each browser tab generates a unique session ID on first load and persists it in `sessionStorage` under the key `poc_cart_session`. This ID scopes all cart and order operations to the session, so multiple tabs operate independently.

```
sess-<timestamp>-<random6chars>
```
